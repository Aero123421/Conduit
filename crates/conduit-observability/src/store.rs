                ...
                for rule in &self.rules {
                    for pattern in &rule.literal_patterns {
                        if !pattern.is_empty() && text.contains(pattern) {
                            let keyed = sha256_bytes(pattern.as_bytes());
                            text = text.replace(
                                pattern,
                                &format!("[REDACTED:{}]", rule.replacement_category),
                            );
                            records.push(record(rule, Some(keyed)));
                        }
                    }
                }
                Value::String(text)
...
impl TraceStore {
    ...
    pub fn append_event(
        ...
        let event = NormalizedEvent {
            ...
            event_type: bound(draft.event_type, 128),
            source_component: bound(draft.source_component, 128),
            observed_at: draft.observed_at,
            monotonic_ns: draft.monotonic_ns,
            boot_id: bound(draft.boot_id, 128),
            correlation_id: bound(draft.correlation_id, 256),
    ...
    pub fn read_events(&self, run_id: &AnyRunId) -> Result<Vec<NormalizedEvent>, TraceError> {
        ...
        let mut paths: Vec<_> = fs::read_dir(directory)?.collect::<Result<Vec<_>, _>>()?;
    ...
    pub fn put_content(
        ...
        for chunk in request.bytes.chunks(MAX_CONTENT_OBJECT_BYTES) {
            ...
            if path.exists() {
                if sha256_bytes(&fs::read(&path)?) != digest {
            ...
            let descriptor_path = self.root.join("objects").join(format!("{digest}.json"));
    ...
    pub fn read_content(
        ...
        if !allow.contains(&descriptor.sensitivity) {
            return Err(TraceError::ContentPermissionDenied);
        }
        let bytes = fs::read(self.root.join(&descriptor.opaque_locator))?;
        if sha256_bytes(&bytes) != descriptor.stored_digest {
    ...
    pub fn recover_partial_segment(
        ...
        Ok((records, (bytes.len() - offset) as u64))
    }
    ...
    pub fn trace_page(
        ...
        Ok(TracePage {
            events: page,
            next_cursor,
            evidence_state: self.verify_chain(run_id)?,
        })
    }
...
fn decode_cursor(value: &str, key: &[u8; 32]) -> Result<CursorCore, TraceError> {
    ...
    if cursor_mac(key, &bytes) != supplied {

...
maximum 4 bound parameters, 144 D1 rows read, 1,005 Durable Object SQL
statements, 8,637 Durable Object rows read, and 69 Durable Object rows written.
The asserted DO ceilings remain 1,152 / 8,784 / 72. The setup's fresh health
frame is intentionally outside that reset measurement; the separate allocation
test observes one fresh send plus 144 exact replays and only one Node outbox
sequence.
...
| Real NodeService + WssClient + Worker route + DeviceRoom, accelerated 24h idle | 144 socket sends and incoming messages; D1 72 statements / 72 binding calls / 144 rows read / 72 rows written / max 4 parameters; measured DO SQL 1,005 statements / 8,637 rows read / 69 rows written, asserted at most 1,152 / 8,784 / 72; 1 ACK row, 6 inbound rows, 0 alarms |

...
Short-lived records have an indexed expiry or explicit archival state. Cleanup deletes a small bounded page and schedules continuation only while backlog remains. Final messages, tool/state/error events, terminal receipts, Reviews, Baselines, and security evidence are never delta-coalesced or dropped.
